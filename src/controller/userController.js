...
const createUser = async function (req, res) {
    try {
        ...
        if (!isValidName(name.trim()))
            return res.status(400)
                .send({ status: false, message: "name is not Valid or Empty" });
        ...
        if (!isValidName(city.trim()))
            return res.status(400)
                .send({ status: false, message: "city name is alpha Numeric" });
        ...
        if (!findcity) {
            return res.status(404).send({
                status: false, message
                    : "Enter City is not found Enter valid city"
            })
        }
        ...
        if (duplicatemobile)
            return res.status(400)
                .send({ status: false, message: "This mobile number is Already Used !" })
